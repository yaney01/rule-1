...
# 保留参数
[bl]     保留: 家宽 ，IPLC 倍率 例如 [0.1x, x0.2, 6x ,3倍]等等标识
[nx]     保留1倍率与不显示倍率的
[blnx]   只保留高倍率
[clear]  清理乱名
...
const regexArray=[/ˣ²/, /ˣ³/, /ˣ⁴/, /ˣ⁵/, /ˣ⁶/, /ˣ⁷/, /ˣ⁸/, /ˣ⁹/, /ˣ¹⁰/, /ˣ²⁰/, /ˣ³⁰/, /ˣ⁴⁰/, /ˣ⁵⁰/, /IPLC/i, /IEPL/i, /核心/, /边缘/, /高级/, /标准/, /实验/, /商宽/, /家宽/, /游戏|game/i, /购物/, /专线/, /LB/, /cloudflare/i, /\budp\b/i, /\bgpt\b|ChatGPT/i,/udpn\b/,/\Netflix|奈飞/i,];
const valueArray= [ "2×","3×","4×","5×","6×","7×","8×","9×","10×","20×","30×","40×","50×","IPLC","IEPL","Kern","Edge","Pro","Std","Exp","Biz","Fam","Game","Buy","Zx","LB","CF","UDP","GPT","UDPN","Netflix"];
    ...
    for (const elem of Object.keys(ik)) {
      if (res.name.indexOf(elem) !== -1) {
        if (!isFgf) {
          ...
          if (addflag) {
            ikey.push(
              gF(us[Object.keys(ik).indexOf(elem)]) +
                FGF +
                namekey +
                ik[elem][0]
            );
