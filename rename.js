...
function oneProxies(proxies){const groups = proxies.reduce((groups, proxy) => { const name = proxy.name.replace(/\D*?\s*?\d+$/, ''); if (!groups[name]) { groups[name] = []; } groups[name].push(proxy);
