...
Promise.all([
  (async () => {
    try {
      const t = await tKey("http://ip-api.com/json/?lang=zh-CN", 1200);
      let e = new Date();
      let s =
        "  " +
        (e.getMonth() + 1) +
      ...
      if (loca) {
        ...
        if (o !== "") {
          ...
          local = "内网: " + o + a + "   " + i + ": " + n.tk + "ms\n";
        } else {
          local = "内网: " + l + a + "   " + i + ": " + n.tk + "ms\n";
        }
      }
      if (bj) {
        const t = await tKey(
      ...
      if (o == pdldip) {
        ...
        if (!c || p) {
          ...
          if (t.code === 0 && t.data.country == "中国") {
            let { province: e, isp: s, city: l } = t.data,
              i = t.tk;
            s = sK(s, 4);
            outik = "入口: " + e + s + ":   " + o + ": " + i + "ms\n";
          } else {
        ...
        if ((!c || isv6) && !cn) {
          const t = await tKey(`http://ip-api.com/json/${o}?lang=zh-CN`, 1e3);
          if (t.status === "success") {
            let { country: e, city: s, org: l, tk: i } = t;
            outik = "入口: " + s + l + ":   " + o + ": " + i + "ms\n";
          } else {
            outik = "入口IPB" + t + "\n";
          }
    ...
    const o = async (r) => {
      try {
        const s = await Promise.race([
          ...
          new Promise((t, s) => {
            setTimeout(() => s(new Error("timeout")), e);
          }),
        ]);
        if (s) {
          i(s);
