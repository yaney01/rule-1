// 转自 适配qx@key @suiyuran
const isQX = typeof $task !== "undefined";
let url = $request.url;
let hasUid = (url) => url.includes("vvebo_uid");
let getUid = (url) => (hasUid(url) ? url.match(/uid=(\d+)/)[1] : undefined);
if (url.includes("users/show")) {
  if (isQX) {
    $prefs.setValueForKey(getUid(url), "vvebo_uid");
  } else {
    $persistentStore.write(getUid(url), "vvebo_uid");
  }
  $done({});
} else if (url.includes("statuses/user_timeline")) {
  let uid = getUid(url);
  if (isQX) {
    uid = $prefs.valueForKey("vvebo_uid");
  } else {
    uid = $persistentStore.read("vvebo_uid");
  }
  url = url
