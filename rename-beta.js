 ...
 */
// const iar = { 'bl':true, 'blkey':'iplc+gpt+NF+IPLC', 'flag':true };
const iar = $arguments;
console.log(iar)
const nx = iar.nx || false,
bl = iar.bl || false,
blnx = iar.blnx || false,
blgd = iar.blgd || false,
blpx = iar.blpx || false,
nf = iar.nf || false,
key = iar.key || false,
debug = iar.debug || false,
clear = iar.clear || false,
addflag = iar.flag || false,
numone = iar.one || false;
...
blkey = iar.blkey == undefined ? " " : decodeURI(iar.blkey);
...
function operator(y) {
    ...
    y.forEach((e) => {
        ...
        const retainKey = blkeyArray.filter(items => e.name.includes(items))
        ...
        if (findKeyValue !== null) {
            ...
            keyover = keyover.concat( usflag, findKeyValue, retainKey, ikey, ikeys).filter(k => k !== '');
            e.name = keyover.join(FGF);
        };
    });
  y = y.filter((e) => e.name !== null); 
  jxh(y);
  numone && oneP(y);
  // blpx && (y = fampx(y));
  key && (y = y.filter((res) => !res.name.match(keyb)));
  return y;
}
