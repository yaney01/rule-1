...
let stops = false;
        ...
        try {
          ...
          if (cacheds) {
            ...
            if (target == "Loon") {
              ...
              timedPush = mTIme(parseInt(readt, 10) - nt + parseInt(loontd, 10))
              } else if(target == "Surge" && Sue){
              timedPush = mTIme(parseInt(readt, 10) - nt + parseInt(innum, 10));
              } else {
              timedPush = mTIme(parseInt(readt, 10) - nt + parseInt(TIMEDKEY, 10));
            }
  ...
  if (target == "Loon") {console.log("缓存过期时间: " + intimed + ", 还剩" + Pushtd.replace(/,/g, ""));
  } else {console.log("缓存过期时间: " + mTIme(TIMEDKEY) + ", 还剩" + Pushtd.replace(/,/g, ""));}
  console.log(`此方法总用时: ${mTIme(timeDiff)}\n----For New CNAME----`);
  // Push
  const readlog = apiRead ? `读取缓存: ${apiRead} 个 ` : '';
  const writelog = apiw ? `写入缓存: ${apiw} 个 ` : '';
