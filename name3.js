...
// argument传入： timeout=数字（单位ms） 设置节点ping超时时间 不传入参数默认为400ms 
...
let names = $arguments.flag ? getFlagEmoji(countryCode) + ' ' + out_info.country : out_info.country;
// 三元运算符
// let names;
// if ($arguments.flag) {
//   names = getFlagEmoji(countryCode) + ' ' + out_info.country;
// } else {
//   names = out_info.country;
// }
...
async function queryIpApi(proxy) {
  ...
  const result = new Promise((resolve, reject) => {
    ...
    // const timeout = $arguments['timeout']; // 判断传入超时 值，单位：ms
    // }else {const timeout = 400;};
